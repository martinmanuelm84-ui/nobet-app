...
export default function CrisisButton({ lang }: { lang: Lang }) {
  ...
  useEffect(() => {
    if (!open) { setSeconds(60); setPhase('wait'); return }
  ...
  const close = () => { setOpen(false); setSeconds(60); setPhase('wait') }
            ...
            {phase === 'wait' ? (
              <>
                ...
                <p className="crisis-message">
                  {ro
                    ? <><strong>60 de secunde.</strong> Atât e craving-ul la vârf. Rămâi cu noi până trece.</>
                    : <><strong>60 seconds.</strong> That's how long a craving peaks. Stay with us until it passes.</>
                  }
                </p>
