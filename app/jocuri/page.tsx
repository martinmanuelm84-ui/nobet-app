...
const GAMES = {
  ro: [
    { icon:'♟️', title:'Șah', desc:'Strategie pură contra calculatorului sau unui prieten. Fiecare mutare contează.', tag:'Strategie', href:'/jocuri/sah', time:'10–30 min' },
    { icon:'🔢', title:'Sudoku', desc:'Completează grila 9×9. Trei dificultăți — ușor, mediu, dificil.', tag:'Logică', href:'/jocuri/sudoku', time:'5–15 min' },
    ...
    { icon:'⬛', title:'Nonograme', desc:'Descoperă imaginea ascunsă rezolvând indicii numerice. Meditativ și satisfăcător.', tag:'Răbdare', href:'/jocuri/nonograme', time:'10–20 min' },
    { icon:'🃏', title:'Memorie', desc:'Găsește toate perechile. Antrenează-ți atenția și memoria de scurtă durată.', tag:'Memorie', href:'/jocuri/memorie', time:'3–8 min' },
  ],
  en: [
    { icon:'♟️', title:'Chess', desc:'Pure strategy against the computer or a friend. Every move matters.', tag:'Strategy', href:'/jocuri/sah', time:'10–30 min' },
    { icon:'🔢', title:'Sudoku', desc:'Complete the 9×9 grid. Three difficulties — easy, medium, hard.', tag:'Logic', href:'/jocuri/sudoku', time:'5–15 min' },
    ...
    { icon:'⬛', title:'Nonograms', desc:'Reveal the hidden image by solving numerical clues. Meditative and satisfying.', tag:'Patience', href:'/jocuri/nonograme', time:'10–20 min' },
...
const READY = ['/jocuri/sah', '/jocuri/sudoku', '/jocuri/2048']
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .page-sub { font-size:1rem; color:var(--text2); line-height:1.75; font-weight:300; max-width:560px; margin-bottom:2.5rem; }
        .craving-banner { background:var(--accent-light); border:1px solid var(--border); border-radius:12px; padding:1.125rem 1.5rem; font-size:0.88rem; color:var(--text2); line-height:1.65; margin-bottom:2.5rem; display:flex; align-items:flex-start; gap:12px; }
        .craving-icon { font-size:1.25rem; flex-shrink:0; margin-top:1px; }
        .games-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:1.25rem; margin-bottom:2.5rem; }
        .game-card { background:var(--surface); border:1px solid var(--border); border-radius:18px; padding:1.75rem 1.5rem; text-decoration:none; color:inherit; display:flex; flex-direction:column; transition:border-color 0.2s, transform 0.2s; cursor:pointer; position:relative; }
        ...
        .soon-badge { position:absolute; top:14px; right:14px; font-size:0.65rem; background:var(--surface2); color:var(--text3); padding:3px 8px; border-radius:100px; font-weight:500; letter-spacing:0.5px; }
      ...
      <div className="jocuri-wrap">
        ...
        <div className="craving-banner">
          ...
          <span>
            {lang==='ro'
              ?'Alege un joc și rămâi cu noi câteva minute. Nu trebuie să faci nimic altceva acum — doar să joci. Dorința trece, tu rămâi.'
              :'Pick a game and stay with us a few minutes. You don\'t have to do anything else right now — just play. The urge passes, you stay.'}
          </span>
        </div>
        ...
        <div className="stats-row">
          <div className="stat-card">
            ...
            <div className="stat-label">{lang==='ro'?'jocuri disponibile':'games available'}</div>
          </div>
          <div className="stat-card">
            <div className="stat-num">0%</div>
            <div className="stat-label">{lang==='ro'?'element de noroc':'element of luck'}</div>
          </div>
          <div className="stat-card">
            <div className="stat-num">15'</div>
            <div className="stat-label">{lang==='ro'?'și dorința trece':'and the urge passes'}</div>
          </div>
        </div>

        <div className="games-grid">
          {games.map((g, i) => {
            const ready = READY.includes(g.href)
            return (
              <div key={i}
                className={`game-card ${ready?'ready':'soon'}`}
                onClick={()=>ready&&router.push(g.href)}>
                {!ready && <span className="soon-badge">{lang==='ro'?'În curând':'Coming soon'}</span>}
